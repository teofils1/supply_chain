...
class ShipmentListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        temp_requirement_filter = self.request.query_params.get('temperature_requirement', None)
        ...
        estimated_delivery_from = self.request.query_params.get('estimated_delivery_from', None)
        estimated_delivery_to = self.request.query_params.get('estimated_delivery_to', None)
        
        if estimated_delivery_from:
            try:
                from datetime import datetime
                est_from_date = datetime.strptime(estimated_delivery_from, '%Y-%m-%d').date()
                queryset = queryset.filter(estimated_delivery_date__date__gte=est_from_date)
            except ValueError:
                pass
        ...
        if estimated_delivery_to:
            try:
                from datetime import datetime
                est_to_date = datetime.strptime(estimated_delivery_to, '%Y-%m-%d').date()
                queryset = queryset.filter(estimated_delivery_date__date__lte=est_to_date)
            except ValueError:
                pass
...
class ShipmentDeleteView(generics.DestroyAPIView):
    ...
    def perform_destroy(self, instance):
        ...
        for shipment_pack in instance.shipment_packs.all():
            ...
            if pack.status in ['shipped', 'delivered'] and not pack.pack_shipments.exclude(
                shipment=instance
            ).filter(shipment__status__in=['picked_up', 'in_transit', 'out_for_delivery', 'delivered']).exists():
                pack.status = 'active'
