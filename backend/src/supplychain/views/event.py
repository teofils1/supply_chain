...
class EventListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if timestamp_from:
            try:
                from_date = datetime.fromisoformat(timestamp_from.replace('Z', '+00:00'))
        ...
        if product_id:
            try:
                ...
                queryset = queryset.filter(
                    ...
                    Q(entity_type='batch', entity_id__in=m.Batch.objects.filter(product_id=product_id).values_list('id', flat=True)) |
                    Q(entity_type='pack', entity_id__in=m.Pack.objects.filter(batch__product_id=product_id).values_list('id', flat=True))
                )
            except (ValueError, TypeError):
                pass
        ...
        if batch_id:
            try:
                ...
                queryset = queryset.filter(
                    ...
                    Q(entity_type='pack', entity_id__in=m.Pack.objects.filter(batch_id=batch_id).values_list('id', flat=True))
                )
            except (ValueError, TypeError):
                pass
        ...
        if shipment_id:
            try:
                ...
                pack_ids = m.ShipmentPack.objects.filter(shipment_id=shipment_id).values_list('pack_id', flat=True)
                queryset = queryset.filter(
        ...
        if alert_only and alert_only.lower() in ['true', '1']:
            queryset = queryset.filter(
                ...
                Q(event_type__in=['temperature_alert', 'damaged', 'recalled', 'error'])
            )
    ...
    def perform_create(self, serializer):
        ...
        if self.request:
            ...
            if not serializer.validated_data.get('user') and self.request.user.is_authenticated:
