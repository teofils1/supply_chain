...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Starting blockchain anchoring process...")
        self.stdout.write(f"Batch size: {batch_size}")
        self.stdout.write(f"Max age: {max_age_hours} hours")
        
        if dry_run:
            self.stdout.write(self.style.WARNING("DRY RUN MODE - No actual anchoring will be performed"))
        ...
        for event in pending_events:
            try:
                if dry_run:
                    self.stdout.write(f"Would anchor event {event.id}: {event.description[:50]}...")
                    continue
                ...
                if result['success']:
                    ...
                    anchored_count += 1
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Anchored event {event.id} - TX: {result['tx_hash']}")
                    )
                else:
                    event.mark_blockchain_failed()
                    failed_count += 1
                    self.stdout.write(
                        self.style.ERROR(f"✗ Failed to anchor event {event.id}: {result.get('error', 'Unknown error')}")
                    )
                ...
                try:
                    ...
                    pass  # Don't fail the entire process if we can't update status
