...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        packs = list(m.Pack.objects.filter(status__in=["active", "shipped", "delivered"]))
        if not packs:
            self.stdout.write(
                self.style.ERROR(
        ...
        self.stdout.write(f"Seeding {options['count']} shipments from {len(packs)} available packs...")
        ...
        origins = [
            {
                ...
                "city": "New York", "state": "NY", "postal_code": "10001", "country": "USA"
            },
            {
                "name": "MediLife Manufacturing",
                "address_line1": "456 Pharma Way",
                "city": "Chicago", "state": "IL", "postal_code": "60601", "country": "USA"
            },
            {
                "name": "Healthcare Solutions Warehouse",
                "address_line1": "789 Medical Drive",
                "city": "Los Angeles", "state": "CA", "postal_code": "90001", "country": "USA"
            },
            {
                "name": "Global Pharma Hub",
                "address_line1": "321 Supply Chain Rd",
                "city": "Atlanta", "state": "GA", "postal_code": "30301", "country": "USA"
            },
            {
                "name": "Northern Distribution",
                "address_line1": "654 Logistics Ave",
                "city": "Boston", "state": "MA", "postal_code": "02101", "country": "USA"
            }
        ]
        ...
        destinations = [
            {
                ...
                "city": "Miami", "state": "FL", "postal_code": "33101", "country": "USA"
            },
            {
                "name": "Regional Medical Center",
                "address_line1": "200 Healthcare Blvd",
                "city": "Dallas", "state": "TX", "postal_code": "75201", "country": "USA"
            },
            {
            ...
            },
            {
                ...
                "city": "Denver", "state": "CO", "postal_code": "80201", "country": "USA"
            },
            {
                "name": "Community Health Network",
                "address_line1": "500 Community Way",
                "city": "Phoenix", "state": "AZ", "postal_code": "85001", "country": "USA"
            },
            {
                "name": "Central Pharmacy",
                "address_line1": "600 Main Street",
                "city": "Kansas City", "state": "MO", "postal_code": "64101", "country": "USA"
            },
            {
                "name": "Coastal Medical Supply",
                "address_line1": "700 Ocean View Dr",
                "city": "San Diego", "state": "CA", "postal_code": "92101", "country": "USA"
            }
        ]

        # Status distribution (weighted)
        statuses = ["pending", "confirmed", "picked_up", "in_transit", "out_for_delivery", "delivered", "returned"]
        ...
        # Service types
        service_types = ["standard", "express", "overnight", "ground", "air", "cold_chain"]
        ...
        with transaction.atomic():
            ...
            for i in range(options["count"]):
                ...
                while m.Shipment.objects.filter(tracking_number=tracking_number).exists():
                    tracking_number = self.generate_tracking_number(carrier)
                ...
                temp_requirement = random.choices(temp_requirements, weights=temp_weights)[0]
                ...
                if status in ["picked_up", "in_transit", "out_for_delivery"]:
                    ...
                    shipped_date = timezone.now() - timedelta(days=random.randint(1, 14))
                    # Estimated delivery 2-7 days after shipping
                    estimated_delivery_date = shipped_date + timedelta(days=random.randint(2, 7))
                    
                elif status == "delivered":
                    # For delivered shipments, work backwards from delivery
                    # Delivered 1-10 days ago
                    actual_delivery_date = timezone.now() - timedelta(days=random.randint(1, 10))
                    # Shipped 3-7 days before delivery
                    shipped_date = actual_delivery_date - timedelta(days=random.randint(3, 7))
                    # Estimated was between shipped and delivered
                    est_days = (actual_delivery_date - shipped_date).days
                    if est_days > 1:
                        estimated_delivery_date = shipped_date + timedelta(days=random.randint(1, est_days))
                    else:
                    ...
                    estimated_delivery_date = timezone.now() + timedelta(days=random.randint(1, 10))
                
                # Special handling and instructions
                special_handling = random.choice([True, False]) if temp_requirement != "ambient" else False
                ...
                notes = random.choice(notes_options) if random.choice([True, False]) else ""
                ...
                try:
                    ...
                    if shipments_created <= 10 or shipments_created % 5 == 0:  # Show first 10 and every 5th
                        self.stdout.write(
                            status_color(
