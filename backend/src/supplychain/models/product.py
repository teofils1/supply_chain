...
class Product(BaseModel):
    ...
    form = models.CharField(
        ...
        help_text="Physical form of the product"
    )
    strength = models.CharField(
        max_length=100,
        blank=True,
        help_text="Product strength (e.g., '500mg', '10ml')"
    )
    ...
    ndc = models.CharField(
        max_length=11,
        blank=True,
        help_text="National Drug Code (NDC)"
    )
    ...
    approval_number = models.CharField(
        max_length=100,
        blank=True,
        help_text="Regulatory approval number"
    )
    ...
    def storage_range_display(self) -> str:
        ...
        if self.storage_humidity_min is not None and self.storage_humidity_max is not None:
            humidity_range = f"{self.storage_humidity_min}% - {self.storage_humidity_max}% RH"
    ...
    def clean(self):
        ...
        if (self.storage_temp_min is not None and 
            self.storage_temp_max is not None and 
            self.storage_temp_min > self.storage_temp_max):
            raise ValidationError({
                'storage_temp_max': 'Maximum temperature must be greater than minimum temperature.'
            })
        
        # Validate humidity range
        if (self.storage_humidity_min is not None and 
            self.storage_humidity_max is not None and 
            self.storage_humidity_min > self.storage_humidity_max):
            raise ValidationError({
