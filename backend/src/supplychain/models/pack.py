...
class Pack(BaseModel):
    ...
    pack_size = models.PositiveIntegerField(
        ...
        help_text="Number of units in this pack"
    )
    pack_type = models.CharField(
        max_length=20,
        choices=PACK_TYPE_CHOICES,
        help_text="Type of packaging"
    )
    ...
    location = models.CharField(
        max_length=255,
        blank=True,
        help_text="Current location/warehouse of the pack"
    )
    warehouse_section = models.CharField(
        max_length=100,
        blank=True,
        help_text="Specific section within the warehouse"
    )
    
    # Quality control and compliance
    quality_control_notes = models.TextField(
        blank=True,
        help_text="Pack-specific quality control notes"
    )
    quality_control_passed = models.BooleanField(
        default=True,
        help_text="Whether the pack passed quality control"
    )
    ...
    regulatory_code = models.CharField(
        max_length=100,
        blank=True,
        help_text="Regulatory tracking code for this pack"
    )
    ...
    shipped_date = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Date and time when the pack was shipped"
    )
    delivered_date = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Date and time when the pack was delivered"
    )
    ...
    def clean(self):
        ...
        elif self.shipped_date and self.status not in ['shipped', 'delivered', 'returned']:
            self.status = 'shipped'
