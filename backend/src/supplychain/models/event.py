...
class Event(BaseModel):
    ...
    location = models.CharField(
        max_length=255,
        blank=True,
        help_text="Location where the event occurred"
    )
    ...
    ip_address = models.GenericIPAddressField(
        null=True,
        blank=True,
        help_text="IP address from which the event was triggered"
    )
    ...
    system_info = models.JSONField(
        default=dict,
        blank=True,
        help_text="System information when the event occurred"
    )
    ...
    def __str__(self):
        entity_info = f"{self.entity_type}#{self.entity_id}" if self.entity_id else "System"
    ...
    def is_alert(self):
        ...
        return self.severity in ['high', 'critical'] or self.event_type in [
            'temperature_alert', 'damaged', 'recalled', 'error'
        ]
    ...
    @classmethod
    def create_event(cls, event_type, entity_type, entity_id, description, 
                    user=None, severity='info', location='', metadata=None, 
                    ip_address=None, user_agent='', system_info=None):
        """
