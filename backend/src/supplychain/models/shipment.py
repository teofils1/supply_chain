...
class Shipment(BaseModel):
    ...
    carrier = models.CharField(
        max_length=20, choices=CARRIER_CHOICES, help_text="Shipping carrier"
    )
    ...
    origin_name = models.CharField(
        max_length=255, help_text="Origin facility or company name"
    )
    origin_address_line1 = models.CharField(
        max_length=255, help_text="Origin address line 1"
    )
    origin_address_line2 = models.CharField(
        max_length=255, blank=True, help_text="Origin address line 2"
    )
    origin_city = models.CharField(max_length=100, help_text="Origin city")
    origin_state = models.CharField(max_length=100, help_text="Origin state/province")
    origin_postal_code = models.CharField(
        max_length=20, help_text="Origin postal/ZIP code"
    )
    origin_country = models.CharField(max_length=100, help_text="Origin country")

    # Destination information
    destination_name = models.CharField(
        max_length=255, help_text="Destination facility or company name"
    )
    destination_address_line1 = models.CharField(
        max_length=255, help_text="Destination address line 1"
    )
    destination_address_line2 = models.CharField(
        max_length=255, blank=True, help_text="Destination address line 2"
    )
    destination_city = models.CharField(max_length=100, help_text="Destination city")
    destination_state = models.CharField(
        max_length=100, help_text="Destination state/province"
    )
    destination_postal_code = models.CharField(
        max_length=20, help_text="Destination postal/ZIP code"
    )
    destination_country = models.CharField(
        max_length=100, help_text="Destination country"
    )

    # Dates
    shipped_date = models.DateTimeField(
        null=True, blank=True, help_text="Date and time when the shipment was shipped"
    )
    estimated_delivery_date = models.DateTimeField(
        null=True, blank=True, help_text="Estimated delivery date and time"
    )
    actual_delivery_date = models.DateTimeField(
        null=True, blank=True, help_text="Actual delivery date and time"
    )
    ...
    special_handling_required = models.BooleanField(
        default=False, help_text="Whether special handling is required"
    )
    ...
    currency = models.CharField(
        max_length=3, default="USD", help_text="Currency for shipping cost"
    )
    billing_reference = models.CharField(
        max_length=100, blank=True, help_text="Billing reference number"
    )

    # Additional information
    notes = models.TextField(
        blank=True, help_text="Additional notes about the shipment"
    )
...
class ShipmentPack(BaseModel):
    ...
    shipment = models.ForeignKey(
        Shipment, on_delete=models.CASCADE, related_name="shipment_packs"
    )
    pack = models.ForeignKey(
        Pack, on_delete=models.CASCADE, related_name="pack_shipments"
    )

    # Additional fields for the relationship
    quantity_shipped = models.PositiveIntegerField(
        default=1, help_text="Quantity of this pack being shipped"
    )
    notes = models.TextField(
        blank=True, help_text="Notes specific to this pack in the shipment"
    )
